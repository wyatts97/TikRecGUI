...
class TikTokAPI:
    ...
    def get_avatar_url(self, room_id: str) -> str | None:
        """
        Given a room_id, I get the owner's avatar thumbnail URL.
        """
        try:
            data = self.http_client.get(
                f"{self.WEBCAST_URL}/webcast/room/info/?aid=1988&room_id={room_id}"
            ...
            avatar = owner.get("avatar_thumb", owner.get("avatar_medium", owner.get("avatar_larger", {})))
    ...
    def get_room_id_from_user(self, user: str) -> str | None:
        ...
        try:
            ...
            if not content or "Please wait" in content:
                logger.warning(f"TikRec WAF/block for {user}, trying fallback")
            ...
            logger.warning(f"TikRec room_id lookup failed for {user}: {exc}")

        # Fallback to eulerstream API
        try:
            params = {"uniqueId": user, "giftInfo": "false"}
            response = self.http_client.get(
                f"{self.EULER_API}/webcast/room_info",
            ...
            logger.warning(f"Eulerstream room_id fallback failed for {user}: {exc}")
